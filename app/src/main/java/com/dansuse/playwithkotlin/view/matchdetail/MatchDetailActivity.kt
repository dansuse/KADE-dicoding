...
class MatchDetailActivity : AppCompatActivity(), MatchDetailView {
      ...
      try {
        database.use {
          insert(FavoriteMatch.TABLE_FAVORITE,
              ...
              FavoriteMatch.AWAY_NAME to event?.awayTeamName,
              FavoriteMatch.HOME_BADGE to event?.homeBadge,
              FavoriteMatch.AWAY_BADGE to event?.awayBadge
          )
        }
        isFavorite = true
  ...
  private fun removeFromFavorite() {
    ...
    try {
      database.use {
        delete(FavoriteMatch.TABLE_FAVORITE, "(${FavoriteMatch.EVENT_ID} = {id})",
            "id" to eventId)
      }
      isFavorite = false
